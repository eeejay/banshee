...
public class StreamChunkReader {
	 ...
	 *                   read errors
	 */
	public static StreamChunk read(FileStream raf, Chunk candidate)
			{
	...
	private StreamChunk parseData(FileStream raf) {
		...
		if (GUID.GUID_STREAM.equals(guid)) {
			...
			if (GUID.GUID_AUDIOSTREAM.equals(streamTypeGUID)
				...
				bool contentEncrypted = (mask & (1 << 15)) != 0;
				...
				if (GUID.GUID_AUDIOSTREAM.equals(streamTypeGUID)) {
					...
					// bitMapInfoHeader
					//					long bitMapInfoHeaderEnd = raf.Position
