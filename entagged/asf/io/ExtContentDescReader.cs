...
public class ExtContentDescReader {
     ...
     *                   Read errors
     */
    public static ExtendedContentDescription read(FileStream raf,
            Chunk candidate) {
     ...
     *                   read errors.
     */
    private ExtendedContentDescription parseData(FileStream raf)
            {
     ...
     *                   read errors.
     */
    private byte[] readBinaryData(FileStream raf) {
    ...
    /**
     * This Method reads a bool value out of the tag chunk. <br>
     * A bool requires 6 bytes. This means we've got 3 16-Bit unsigned
     ...
     *                   read errors.
     */
    private bool readBoolean(FileStream raf) {
        int size = Utils.readUINT16(raf);
        if (size != 4)
